%Rotasi melakukan operasi pemutaran dengan hasil utuh citra
F = imread ('D:\Citra Digital\citra digital\Grafiti.jpg');
ukuran = size(F);
tinggi = ukuran (1);
lebar = ukuran (2);

sudut = 45; %sudut pemutar
rad = pi * sudut/180;
cosa = cos(rad);
sina = sin(rad);
...
for y=1 : tinggi_baru
    for x=1 : lebar_baru
        ...
        if(x2>=1) && (x2<=lebar_baru) && (y2>=1) && (y2<=tinggi_baru)
            G(y, x) = F2 (y2, x2);
        else
            G(y, x) = 0;
        end
...
figure(2); imshow(G); title('Citra Putar');
clear all;